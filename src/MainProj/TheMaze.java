...
public class TheMaze<T> {

	public MNode<Character> root = new MNode<Character>('B', 0);
	...
	public void addPathToTree(String pathST) {
		...
		for (int i = 0; i < pathRL.length(); i++) {
			if (pathRL.charAt(i) == 'L') {
				if (tmp.left == null)
					tmp.left = new MNode<Character>(pathST.charAt(i), lastkey++);
				tmp = tmp.left;
			} else if (pathRL.charAt(i) == 'R') {
				if (tmp.right == null)
					tmp.right = new MNode<Character>(pathST.charAt(i), lastkey++);
				tmp = tmp.right;
	...
	private boolean follow(MNode<Character> t, String path) {
		String pathRL = translateToLeftRight(path);

		MNode<Character> tmp = t;
		for (int i = 0; i < pathRL.length(); i++) {
	...
	private boolean escape(MNode<Character> t) {
		if (t == null)
			return false;
		if (t.data.equals('X'))
			return true;
	...
	private String findShortest(MNode<Character> t) {
		String left = "";
		String right = "";
